...
const HELP_HTML = `
  <article>
    ...
    <ol>
      ...
      <li><strong>Playback</strong>: pick a <em>Voice</em>, adjust <em>Speed</em> and <em>Volume</em>, then toggle: <em>Loop</em>, <em>Shuffle</em>, <em>Words</em>, <em>Examples</em>, <em>Include translations</em>. Click <strong>Auto-play</strong>.</li>
...
function starHtml(value, deckIndex) {
    const stars = Array.from({ length: 5 }, (_, i) => {
        const starValue = i + 1;
        return `<span class="star ${starValue <= value ? 'on' : ''}" data-v="${starValue}">★</span>`;
    });
...
export function renderDeckTable({ deck, filteredIndices, tbody, table, exportBtn, getFam, onSpeak, onFamiliarityChange }) {
    ...
    tbody.querySelectorAll('.speak').forEach((el) => {
        el.addEventListener('click', () => {
            ...
            if (Number.isInteger(index) && field) onSpeak?.(index, field);
        });
    });
    ...
    tbody.querySelectorAll('.stars').forEach((el) => {
        el.addEventListener('click', (event) => {
            const index = Number(el.dataset.i);
            const value = Number(event.target.dataset.v ?? el.dataset.v);
            if (Number.isInteger(index) && Number.isInteger(value)) {
                onFamiliarityChange?.(index, value);
            }
        });
    });
}
